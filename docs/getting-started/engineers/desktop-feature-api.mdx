...
Use this to send an [exposure event](/jetstream/jetstream/#enrollment-vs-exposure). By default this will send one exposure event per function call, but you can add an options object of `{once: true}` to only send it once per session.
