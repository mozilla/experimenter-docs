...
[fxios-trigger-expressions]:     #missing-link
[fxios-trigger-expressions-eng]: #missing-link

[fenix-actions]:                 #missing-link
...
[fenix-trigger-expressions]:     #missing-link
[fenix-trigger-expressions-eng]: #missing-link
...
![my-first-message in the branch configuration screenn](/img/gleanplumb/branch-config.png)
...
The app provides the messaging system with attributes. We use JEXL expressions to query these attributes.
...
- `max-display-count` - the number of sessions the message is shown before expiring.
- `style` – this is a string key into the `styles` object. It will code for visual style, and/or priority.

## Localization of messages

Localization of strings and messages is not fully developed in Nimbus,
...
However this relies on the exact translations either being coincidentally already in use by the app, or the experimental copy known about before the app was released.
...
Deeplinks are implemented by the application developers, and should be added to the `nimbus.fml.yaml` file.

- Firefox for Android [Deeplinks in the `AndroidManifest.xml`][fenix-actions-eng].
- Firefox for iOS [Deeplinks in the `NavigationRouter.swift`][fxios-actions-eng].
...
It is possible this table is out of date. The definitive source of truth for this in [the code][fenix-trigger-expressions] itself.
...
It is possible this table is out of date. The definitive source of truth for this in [the code][fxios-trigger-expressions] itself.
