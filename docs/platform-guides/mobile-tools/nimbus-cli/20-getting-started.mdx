...
slug: /technical-reference/nimbus-cli/getting-started
---

This guide introduces the basic commands for listing experiments, enrolling in branches, resetting the app, and getting experiment information. Most commands require `--app` and `--channel` parameters that correspond to the installed app.
...
By default, the experiment branches will be validated against the feature manifest. This [can be configured with `--version VERSION`, `--ref REF` and `--manifest FILE` parameters](/technical-reference/nimbus-cli/fml), or disabled with the `--no-validate` parameter.
...
You can copy the slug to the clipboard in Experimenter by pressing the <img src="/img/technical-reference/nimbus-cli/copy.svg" width="20"/> icon next to the experiment slug at the top of the summary page:

<img title="Clicking on the copy to clipboard button shows a Copied to Clipboard tooltip" src="/img/technical-reference/nimbus-cli/copy-to-clipboard-copied.png" width="40%"/>
