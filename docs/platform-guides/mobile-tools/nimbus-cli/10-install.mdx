...
title: Installation
slug: /technical-reference/nimbus-cli/install
---

The `nimbus-cli` is a command line tool to help develop and test experiments and features on [an instrumented mobile app](/technical-reference/nimbus-cli/instrumentation).
...
<img title="Show in Finder" src="/img/technical-reference/nimbus-cli/show-in-finder-dialog.png"/>

- Click on "Show in Finder"
- Right click on the `nimbus-cli` icon
- Select "Open"

<img title="Open anyway dialog" src="/img/technical-reference/nimbus-cli/open-anyway-dialog.png"/> 

- Confirm that you Trust this Developer.

## Installing Nimbus CLI on Windows
...
```sh
cargo build --manifest-path components/support/technical-reference/nimbus-cli/Cargo.toml --
```
...
```sh
alias nimbus-cli="cargo run --manifest-path $(pwd)/components/support/technical-reference/nimbus-cli/Cargo.toml --"
```
