...
`nimbus-cli` assumes that [`adb` and `xcrun` are on the `PATH`](/technical-reference/nimbus-cli/install#rerequisites). If they are not (or aren't spelled `adb` or `adb.exe`), then you can configure them via the `ADB_PATH` and `XCRUN_PATH` environment variables.
