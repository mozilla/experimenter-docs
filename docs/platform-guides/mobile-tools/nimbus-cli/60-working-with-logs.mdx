...
title: Logs
slug: /technical-reference/nimbus-cli/working-with-logs
---

This guide covers capturing logs, tailing logs in real-time, and logging the Nimbus SDK state. The CLI uses `adb logcat` on Android and Unix commands on iOS to access device logs.
...
You can do the same as `enroll` then `tail-logs` with by adding [`-- --console-pty {}`](/technical-reference/nimbus-cli/opening-and-resetting#leading-parameters) to the end of the enroll command.
