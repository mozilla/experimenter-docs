...
title: Opening & Resetting
slug: /technical-reference/nimbus-cli/opening-and-resetting
---

This guide covers opening apps with deeplinks, terminating and resetting apps, and using passthrough parameters for advanced control. Several commands simulate the app launch as if the user tapped on the app icon.
...
Opening with a deeplink while enrolling in an experiment is supported but requires a different route. See [Working with Devices](/technical-reference/nimbus-cli/on-devices).

:::

### Enrolling With Deeplinks
...
### Testing Features With Deeplinks

Similarly, you can use a `--deeplink` to enroll a [`test-feature`](/technical-reference/nimbus-cli/working-with-features) experiment.
