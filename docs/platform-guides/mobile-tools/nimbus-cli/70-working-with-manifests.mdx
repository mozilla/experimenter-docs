...
title: Manifests
slug: /technical-reference/nimbus-cli/fml
---

This guide covers validating experiments against feature manifests, extracting default configurations, and working with the Feature Manifest Language. The CLI can validate experiments before they launch using the app's feature manifest.
...
## Merging Features With Defaults
...
The Feature Manifest Language has its own [command line interface](/technical-reference/fml/fml-cli). This can be used to validate manifest files, generate code, and rationalize manifests in to a single file.
...
[The documentation there is better placed to explain how to use it](/technical-reference/fml/fml-cli), however there is one caveat: flags (i.e. modifiers that begin with `--`) are caught by the `nimbus-cli` command line, rather than passed to the `nimbus-fml` command line.
