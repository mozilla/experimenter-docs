...
Use `preview`, `stage` or `stage/preview` to specify a Remote Settings instance with the `--server` parameter:

```sh
nimbus-cli --app fenix --channel developer fetch --server preview archived.json
```

Alternatively, you can specify one or more recipes, using the `--recipe` option:
