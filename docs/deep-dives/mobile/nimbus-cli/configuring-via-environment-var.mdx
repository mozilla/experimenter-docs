...
The app supposes that `adb` and `xcrun` are on the `PATH`. If they are not (or aren't spelled `adb` or `adb.exe`), then you can configure them via the `ADB_PATH` and `XCRUN_PATH` environment variables.

```sh
ADB_PATH=C:\Program-Tools\bin\adb.exe nimbus-cli
```
