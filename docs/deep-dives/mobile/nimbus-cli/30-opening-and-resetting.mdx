...
`nimbus-cli` knows the apps' deeplinking schemes, and if no `://` is detected in the deeplink, then the deeplink scheme is prepended.
...
By default, each of the commands that launches the app also kills the app right before; with the `open` command you can send the app a deeplink without killing it, using the `--no-clobber` option.
