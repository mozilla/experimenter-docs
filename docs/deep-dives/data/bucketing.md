...
As an example, suppose that 75% of the namespace has been consumed by experiments that _have ended_ and that 20% of the namespace is consumed by an active experiment (e.g., the most recent experiment is still live with a `bucketConfig` of `{"start":7500, "end":9500", "total":10000}`). Suppose further that another 20% experiment is launched. In that situation, the namespace will rollover and buckets `0000` to `2000` of the new namespace will be consumed by the new experiment. Some fraction of the clients in that range (approximately 20%) will still be enrolled into the previous experiment, so even though they meet the enrollment criteria, they will not enroll. So this experiment will actually enroll 16% of the available userspace, even though it targeted 20%.
