...
In order to [record exposure](/cookbook/growable-collections#exposure-events) for a co-enrolled feature, the string `{experiment}` needs to exist in the feature config somewhere. In the case of messaging, this would be in the message object:
