...
slug: /messaging/mobile-messaging
sidebar_position: 7
---
...
The app provides the messaging system with [attributes](#list-of-attributes). We use JEXL expressions to query these attributes, or trigger messages based on [the user's behavior in the app](/mobile-behavioral-targeting).
...
Much of the system relies on Nimbus [merging together JSON objects](/cookbook/growable-collections). We have seen this in the `messages` object which can contain messages from the default configuration, rollouts, and experiments.
