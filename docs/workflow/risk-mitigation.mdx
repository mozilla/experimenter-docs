...
The only possible exceptions where you may not need official QA are below; **if you choose to skip QA, you must self-test**, [learn more](/platform-guides/desktop/preview). Self-testing is recommended even before sending to QA, as it can avoid days of delay if QA discovers an easily corrected recipe error when they start testing.
